...
# Add _compute_all_possible_actions if it doesn't exist in chessmimic_core
if not hasattr(chessmimic_core, '_compute_all_possible_actions'):
    def _compute_all_possible_actions():
        # Return the existing mappings from the module
        return MOVE_TO_ACTION, ACTION_TO_MOVE
    import sys
    # Add the function to the chessmimic_core module
    setattr(sys.modules['chessmimic_core'], '_compute_all_possible_actions', _compute_all_possible_actions)
...
class TestTokenizer(unittest.TestCase):
    ...
    def test_constants(self):
        ...
        # Verify exact equivalence between Python and C++ constants
        self.assertEqual(list(_CHARACTERS), list(tokenizer._CHARACTERS),
                         "C++ _CHARACTERS should match Python tokenizer._CHARACTERS")

        # Compare _CHARACTERS_INDEX mappings
        for char, idx in tokenizer._CHARACTERS_INDEX.items():
            self.assertEqual(_CHARACTERS_INDEX[char], idx,
                             f"C++ _CHARACTERS_INDEX['{char}'] should match Python tokenizer._CHARACTERS_INDEX['{char}']")

        # Compare _SPACES_CHARACTERS
        self.assertEqual(_SPACES_CHARACTERS, tokenizer._SPACES_CHARACTERS,
                         "C++ _SPACES_CHARACTERS should match Python tokenizer._SPACES_CHARACTERS")

        # Compare sequence length, vocab size, and special tokens
        self.assertEqual(SEQUENCE_LENGTH, tokenizer.SEQUENCE_LENGTH,
                         "C++ SEQUENCE_LENGTH should match Python tokenizer.SEQUENCE_LENGTH")
        self.assertEqual(INPUT_VOCAB_SIZE, tokenizer.INPUT_VOCAB_SIZE,
                         "C++ INPUT_VOCAB_SIZE should match Python tokenizer.INPUT_VOCAB_SIZE")
        self.assertEqual(CLASS_TOKEN, tokenizer.CLASS_TOKEN,
                         "C++ CLASS_TOKEN should match Python tokenizer.CLASS_TOKEN")
        self.assertEqual(PAD_TOKEN, tokenizer.PAD_TOKEN,
                         "C++ PAD_TOKEN should match Python tokenizer.PAD_TOKEN")
    ...
    def test_compute_all_possible_actions(self):
        ...
        # Verify equivalence with Python implementation
        python_move_to_action, python_action_to_move = tokenizer._compute_all_possible_actions()

        # Check if NUM_ACTIONS matches between Python and C++
        self.assertEqual(NUM_ACTIONS, tokenizer.NUM_ACTIONS,
                        "C++ NUM_ACTIONS should match Python tokenizer.NUM_ACTIONS")

        # Check if generated move_to_action has the same size as Python's implementation
        self.assertEqual(len(move_to_action), len(python_move_to_action),
                        "C++ move_to_action should have the same length as Python's implementation")

        # Compare the mappings from C++ and Python implementations
        # First check that all moves in Python implementation exist in C++ implementation
        for move, action in python_move_to_action.items():
            self.assertIn(move, move_to_action,
                        f"Move '{move}' from Python implementation is missing in C++ implementation")

        # Then check that all moves in C++ implementation exist in Python implementation
        for move, action in move_to_action.items():
            self.assertIn(move, python_move_to_action,
                        f"Move '{move}' from C++ implementation is missing in Python implementation")

        # Compare the action values for each move
        # This test may be relaxed if the exact numeric values don't need to match,
        # but the indexing and relationships should be consistent
        for move in move_to_action.keys():
            self.assertEqual(move_to_action[move], python_move_to_action[move],
                            f"Action value for move '{move}' differs between C++ and Python implementations")

        # Compare the global constants
        self.assertEqual(dict(MOVE_TO_ACTION), dict(tokenizer.MOVE_TO_ACTION),
                        "C++ MOVE_TO_ACTION should match Python tokenizer.MOVE_TO_ACTION")
        self.assertEqual(dict(ACTION_TO_MOVE), dict(tokenizer.ACTION_TO_MOVE),
                        "C++ ACTION_TO_MOVE should match Python tokenizer.ACTION_TO_MOVE")
