...
class ClockPatzerModel(pl.LightningModule):
    """Clock prediction model based on PatzerModel architecture with discrete-only output."""
    ...
    def __init__(
        ...
        self.learned_positional_encoding = nn.Parameter(
            torch.randn(recent_moves_sequence_length + board_sequence_length + 2, embedding_dim))  # +2 for rating and clock
...
class LightningModel(pl.LightningModule):
    def __init__(self, clock_model, bucket_info,
        ...
        # Metrics
        self.train_accuracy = Accuracy(num_classes=bucket_info.n_buckets, task="multiclass")
        self.train_mae = MeanMetric()
        
        self.val_accuracy = Accuracy(num_classes=bucket_info.n_buckets, task="multiclass")
    ...
    def forward(self, input_ids, attention_mask, scaled_rating, clock_features):
        return self.model(input_ids, attention_mask, scaled_rating, clock_features)
    
    @staticmethod
    def logits_to_probabilities(logits, bucket_mask):
        """Convert logits to probabilities with bucket masking."""
        # Apply mask to logits
        masked_logits = logits.masked_fill(~bucket_mask, float('-inf'))
        
        # Compute probabilities
        probabilities = F.softmax(masked_logits, dim=-1)
        
        # No need to renormalize as softmax handles -inf values correctly
        return probabilities

    def _compute_metrics(self, logits, target_bucket, raw_thinking_time, prefix):
        """Compute and log metrics for a batch."""
        predictions = logits.argmax(dim=-1)
        
        # Accuracy
        acc_metric = self.train_accuracy if prefix == "train" else self.val_accuracy
        acc_metric(predictions, target_bucket)
        
        # MAE in seconds
        pred_times = torch.zeros_like(raw_thinking_time)
        for i in range(len(predictions)):
            pred_times[i] = self.bucket_info.get_bucket_center(predictions[i].item())
        
        mae = torch.mean(torch.abs(pred_times - raw_thinking_time))
        mae_metric = self.train_mae if prefix == "train" else self.val_mae
        mae_metric(mae)
    ...
    def training_step(self, batch, batch_idx):
        logits = self(
            ...
            batch['rating'], 
            batch['clock_features']
        )
        
        # Use bucket mask from dataset
        bucket_mask = batch['bucket_mask']
        
        # Masked Brier loss
        loss = self.loss(logits, batch['target_bucket'], bucket_mask)
        ...
        # Convert to probabilities with masking for metrics
        probabilities = self.logits_to_probabilities(logits, bucket_mask)
        predictions = probabilities.argmax(dim=-1)
        
        # Compute metrics using masked predictions
        self.train_accuracy(predictions, batch['target_bucket'])
        
        # Compute MAE
        pred_times = torch.zeros_like(batch['raw_thinking_time'])
        for i in range(len(predictions)):
            pred_times[i] = self.bucket_info.get_bucket_center(predictions[i].item())
        mae = torch.mean(torch.abs(pred_times - batch['raw_thinking_time']))
        self.train_mae(mae)
        
        # Log metrics
        self.log('train/accuracy_step', self.train_accuracy)
        self.log('train/mae_step', self.train_mae)
    ...
    def validation_step(self, batch, batch_idx):
        ...
        # Compute metrics using masked predictions
        self.val_accuracy(predictions, batch['target_bucket'])
    ...
    def on_train_epoch_end(self):
        ...
        self.log('train/mae_epoch', self.train_mae.compute(), sync_dist=False)
    ...
    def on_validation_epoch_end(self):
        ...
        self.log('val/mae_epoch', self.val_mae.compute(), sync_dist=False)
...
def load_pretrained_weights(clock_model, checkpoint_path):
    """Load weights from a pre-trained PatzerModel checkpoint.
    
    Handles architectural differences:
    - Clock embedding: 1D -> 3D (reuses weights for first dimension, duplicates for others)
    - Output layer: Single head -> classification head (skipped, uses random init)
    """
    ...
    current_state = clock_model.state_dict()
    ...
    for key, value in state_dict.items():
        if key in current_state:
            if key == 'clock_time_embedding.weight':
                # Special handling for clock embedding dimension mismatch
                # Original is (embedding_dim, 1), new is (embedding_dim, 3)
                # Strategy: Use pretrained weights for player clock, duplicate for opponent clock and increment
                pretrained_weights = value.squeeze(-1)  # Shape: (embedding_dim,)
                current_state[key][:, 0] = pretrained_weights  # Player clock
                current_state[key][:, 1] = pretrained_weights  # Opponent clock (same init)
                current_state[key][:, 2] = pretrained_weights * 0.1  # Increment (scaled down)
                special_handling.append(f"{key}: 1D->3D (duplicated with scaling)")
            elif key == 'clock_time_embedding.bias' and value.numel() == current_state[key].numel():
                # Handle bias term (same for all 3 inputs)
                current_state[key] = value
                loaded_keys.append(key)
            elif key.startswith('output_layer') or key.startswith('discrete_head') or key.startswith('continuous_') or key.startswith('mode_'):
            ...
            skipped_keys.append(f"{key} (not in clock model)")
    
    # Load the modified state dict
    clock_model.load_state_dict(current_state)
    
    # Print detailed loading summary
    print(f"\nPre-trained checkpoint loading summary:")
    print(f"  Total keys in checkpoint: {len(state_dict)}")
    print(f"  Total keys in clock model: {len(current_state)}")
    ...
    not_in_model = [sk for sk in skipped_keys if 'not in clock model' in sk]
    ...
    if not_in_model:
        print(f"    - {len(not_in_model)} layers not in clock model")
    if shape_mismatch:
...
def main(args):
    ...
    train_dataset = ClockDataset.ClockDataset(
        bagz_path=args.train_bagz_path, 
        scalers=scalers,
        bucket_boundaries_path=args.bucket_boundaries
    )
    print(f"Train dataset length: {len(train_dataset)}")
    print(f"Number of buckets: {train_dataset.get_n_buckets()}")
    
    # Get sample to check shapes
    sample = train_dataset[0]
    print(f"Input IDs shape: {sample['input_ids'].shape}")
    print(f"Clock features shape: {sample['clock_features'].shape}")
    ...
    if args.val_bagz_path is None:
        ...
        val_dataset = ClockDataset.ClockDataset(
    ...
    # Create clock model with dynamic bucket count
    model = ClockPatzerModel(
    ...
    # Get bucket info from the dataset
    ...
    if not args.disable_wandb:
        ...
        wandb_logger = WandbLogger(project="ChessClock", name=args.tensorboard_name, log_model=log_model)
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(description='Train a discrete clock prediction model using bagz datasets')
    
    # Dataset arguments
    parser.add_argument('--train_bagz_path', type=str, help='Path to the training clock bagz file', required=True)
    parser.add_argument('--val_bagz_path', type=str, help='Path to the validation clock bagz file', required=False)
    parser.add_argument('--scalers', type=str, help='Path to the clock scalers pickle file', required=True)
    parser.add_argument('--bucket_boundaries', type=str, help='Path to bucket boundaries JSON file', required=True)
